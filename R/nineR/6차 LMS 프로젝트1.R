...
totConfirmed20 <- finalDF[order(finalDF$Confirmed.x,finalDF$Deaths.x,finalDF$MeanConfirmed,finalDF$MeanDeaths,decreasing = TRUE),]
...
finalDF%>%subset(finalDF$Country=="Korea, South")
...
#########단순 뺀것과 합계낸것 검증#############
###########정규성 검정###############
# 귀무가설(H0):두 집단 간 차이는 없다.
# 대립가설(H1):두 집단 간 차이는 있다.
...
qqnorm(test_sum_DF$Confirmed, ylim = c(0,4.0e+06), xlim = c(-3, 2))
...
##### 사망률 = (사망자/확진자)*100######
test_sum_DF$DR <- (test_sum_DF$Deaths.x/test_sum_DF$Confirmed.x)*100
test_sum_DF <- test_sum_DF[order(test_sum_DF$DR,test_sum_DF$Confirmed.x,test_sum_DF$Deaths.x,decreasing = TRUE),]
test_sum_DF
