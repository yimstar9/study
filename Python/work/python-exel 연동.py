...
# 코드 3-1 Create Excel Workbook


from openpyxl import Workbook

filename = '사원_정보2.xlsx'
...
# ch3 코드 3-4 리스트형 자료를 열 단위로 이어 붙이기
...
# Default is None
# itemgetter(0) : index 0을 기준으로 sort하겠다는 의미
...
# ch3 코드 3-5 행 단위로 각 열에 맞는 데이터 추가 삽입


from openpyxl import load_workbook
from openpyxl.utils import get_column_letter

filename = '사원_정보3.xlsx'
...
# idx에 해당하는 행 바로 앞에 col 추가
personnel_info_ws.insert_cols(5, 1)
# personnel_info_ws.insert_cols(5,2) # 행 2개
...
filename4 = '사원_정보5.xlsx'
df.to_excel(filename4)

# 여러 worksheet 를 각각 dataframe으로 만들어 excel로 저장
...
# 코드 3-7 규칙파일을 읽어서 해당 내용을 반환
# 코드 3-8 get_rules함수 실행


def get_rules(filename):
    file = open(filename, 'r',encoding='UTF8')
    key_column = file.readline().split(':')[1]  # 1로 수정
...
# ch3 코드 3-9 워크북에서 원하는 열을 찾아서 반환
